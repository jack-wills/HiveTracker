...
Mat Counter::drawCounter(Mat image) {
    ...
    putText(image, to_string(enterCounter), Point(imgWidth + 30, 80), 3, FONT_HERSHEY_DUPLEX , Scalar(0, 0, 255));
    putText(image, to_string(exitCounter), Point(imgWidth - 60, 80), 3, FONT_HERSHEY_DUPLEX , Scalar(0, 0, 255));
    color = Scalar(0, 0, 255);
    return image;
}
